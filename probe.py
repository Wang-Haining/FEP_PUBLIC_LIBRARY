...
def probe(df, mode="content", max_features=120, model_name=None):
    ...
    Note: We place reference groups last in the encoding for statsmodels compatibility
    while maintaining their conceptual position as class 0 in the output.
    ...
    # Store mapping for statsmodels output
    original_class_map = {}

    le = LabelEncoder()
    # Reorder labels to put reference groups at the end for statsmodels
    if set(df["label"].unique()) == {"F", "M"}:
        le.classes_ = np.array(["M", "F"])  # Female at end (reference)
        original_class_map = {0: 1}  # M encoded as 0 but maps to 1
    elif set(df["label"].unique()) == {
        "White",
        "Black or African American",
        "Asian or Pacific Islander",
        ...
        original_class_map = {0: 1, 1: 2, 2: 3, 3: 4, 4: 5}
    elif set(df["label"].unique()) == {
        "Undergraduate student",
        "Faculty",
        "Graduate student",
    ...
    # statsmodels with simplified processing
    try:
        # adjust feature size only for Gemma-2 content mode
        if model_name and "gemma" in model_name.lower() and mode == "content":
            vectorizer_stats = TfidfVectorizer(
        ...
        if n_classes == 2:
            ...
            # For binary classification, map to original class 1
            stats_df = pd.DataFrame({
                'feature': [feat_const[i] for i in range(len(mask)) if mask[i]],
                'class': '1',  # Male (conceptually class 1)
                'coef': params[mask],
            ...
            # Pre-compute feature and class lists more efficiently
            n_feat = len(feat_const)
            n_target = n_classes - 1

            # Create arrays directly rather than appending in loops
            features = np.repeat(feat_const, n_target)

            # Faster class mapping with vectorized operations
            encoded_classes = np.tile(np.arange(n_target), n_feat)
            mapped_classes = np.array([original_class_map.get(c, c) for c in encoded_classes])

            # Build dataframe directly
            data = {
                'feature': features,
                'class': [str(c) for c in mapped_classes],
                'coef': params,
                'p_value': pvals
            }
            ...
            stats_df = stats_df[~np.isnan(stats_df['coef'])]

        # Remove constant term and sort by absolute coefficient
        stats_df = stats_df[stats_df.feature != 'const']
        stats_df.reset_index(drop=True, inplace=True)

        # Sort more efficiently using numpy operations
        abs_coef = np.abs(stats_df['coef'].values)
        sort_idx = np.argsort(-abs_coef)  # Descending order
        stats_df = stats_df.iloc[sort_idx].reset_index(drop=True)
